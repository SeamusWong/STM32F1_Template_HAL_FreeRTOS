...
UART_HandleTypeDef V_config_usart1;

void InitializeUSART1ForProtocol(void)
{
    FUNC_USART_1_CLK_ENABLE();

    V_config_usart1.Instance = CONFIG_USART1_INSTANCE;
    V_config_usart1.Init.Mode = CONFIG_USART1_MODE;
    V_config_usart1.Init.BaudRate = CONFIG_USART1_BAUDRATE;
    V_config_usart1.Init.WordLength = CONFIG_USART1_WORDLENGTH;
    V_config_usart1.Init.StopBits = CONFIG_USART1_STOPBITS;
    V_config_usart1.Init.Parity = CONFIG_USART1_PARITY;
    V_config_usart1.Init.HwFlowCtl = CONFIG_USART1_HWCONTROL;
    // V_config_usart1.Init.CLKLastBit = CONFIG_USART1_CLKLASTBIT;
    // V_config_usart1.Init.CLKPhase = CONFIG_USART1_CLKPHASE;
    // V_config_usart1.Init.CLKPolarity = CONFIG_USART1_CLKPOLARITY;
    FUNC_CHECK_FOR_HAL(HAL_UART_Init(&V_config_usart1), "ERROR UART_Init");

    __HAL_UART_ENABLE(&V_config_usart1);
}
...
//     {
//         HAL_UART_Transmit(&V_config_usart1, (uint8_t *)&m_str[count_char], sizeof m_str[count_char], CONFIG_USART1_TIMEOUT);
//     }

//     FuncLED1SetColour(STATUS_LED_COLOUR_YELLOW);
//     FuncDelayMicrosecondsForAll(1000);
//     FuncLED1SetColour(STATUS_LED_COLOUR_BLACK);
//     FuncDelayMicrosecondsForAll(1000);

//     FUNC_WAITSTATUS_TIMEOUT(__HAL_UART_GET_FLAG(&V_config_usart1, USART_FLAG_TC),RESET,CONFIG_USART1_TIMEOUT,"Timeout USART1");
// }
...
{
    HAL_UART_Transmit(&V_config_usart1, (uint8_t *)&m_char, sizeof((uint8_t)m_char), CONFIG_USART1_TIMEOUT);
...
{
    uint8_t m_char;
    HAL_UART_Receive(&V_config_usart1, (uint8_t *)&m_char, sizeof((uint8_t)m_char), CONFIG_USART1_TIMEOUT);
 ...
 * @description: I2C区段
 */
I2C_HandleTypeDef V_config_iic1;
...
void InitializeIIC1ForProtocol(void)
{
    ...
    V_config_iic1.Instance = I2C1;
    V_config_iic1.Init.ClockSpeed = CONFIG_IIC1_CLOCKSPEED;
    V_config_iic1.Init.AddressingMode = CONFIG_IIC1_ADDRESSINGMODE;
    V_config_iic1.Init.DualAddressMode = CONFIG_IIC1_DUALADDRESSMODE;
    V_config_iic1.Init.DutyCycle = CONFIG_IIC1_DUTYCYCLE;
    V_config_iic1.Init.GeneralCallMode = CONFIG_IIC1_GENERALCALLMODE;
    V_config_iic1.Init.NoStretchMode = CONFIG_IIC1_NOSTRETCHMODE;
    V_config_iic1.Init.OwnAddress1 = CONFIG_IIC1_OWNADDRESSMODE_1;
    V_config_iic1.Init.OwnAddress2 = CONFIG_IIC1_OWNADDRESSMODE_2;

    FUNC_CHECK_FOR_HAL(HAL_I2C_Init(&V_config_iic1), "ERROR IIC1_Init");

    __HAL_I2C_ENABLE(&V_config_iic1);
}

void FuncIIC1WaitForReady(uint16_t m_target_address)
{
    FUNC_WAITSTATUS_TIMEOUT(HAL_I2C_GetState(&V_config_iic1), HAL_I2C_STATE_READY, CONFIG_IIC1_TIMEOUT, "ERROR IIC1_Timeout");

    FUNC_CHECK_FOR_HAL(HAL_I2C_IsDeviceReady(&V_config_iic1, m_target_address, 1000, CONFIG_IIC1_TIMEOUT), "ERROR IIC1_Timeout_DeviceReady");
...
void FuncIIC1Transmit(uint16_t m_target_address, uint8_t *m_buffer_ptr, uint16_t m_num_to_write)
{
    ...
    HAL_I2C_Master_Transmit(&V_config_iic1, m_target_address, m_buffer_ptr, m_num_to_write, CONFIG_IIC1_TIMEOUT);
    FuncIIC1WaitForReady(m_target_address);
}
...
{
    FuncIIC1WaitForReady(m_target_address);
    HAL_I2C_Master_Receive(&V_config_iic1, m_target_address, m_buffer_ptr, m_num_to_read, CONFIG_IIC1_TIMEOUT);
    FuncIIC1WaitForReady(m_target_address);
}
...
void FuncIIC1WriteForMemory(uint16_t m_target_address, uint16_t m_target_pagesize, uint8_t *m_buffer_ptr, uint8_t m_target_mem_ptr, uint16_t m_num_to_write)
{
    ...
    {
        flag = HAL_I2C_Mem_Write(&V_config_iic1,             /*指向 I2C_HandleTypeDef 配置结构的指针*/
...
{
    HAL_StatusTypeDef flag = HAL_OK;
    flag = HAL_I2C_Mem_Read(&V_config_iic1,             /*指向 I2C_HandleTypeDef 配置结构的指针*/
                            m_target_address,           /*目标设备地址*/
                            (uint16_t)m_target_mem_ptr, /*目标内部存储器地址*/
                            I2C_MEMADD_SIZE_8BIT,       /*内存地址大小*/
                            m_buffer_ptr,               /*准备发送的缓冲区指针*/
 ...
 * @description: SPI区段
 */
SPI_HandleTypeDef V_config_spi1;
...
void InitializeSPI1ForProtocol(void)
{
    ...
    V_config_spi1.Instance = SPI1;
    V_config_spi1.Init.BaudRatePrescaler = CONFIG_SPI1_BAUDRATEPRESCALER;
    V_config_spi1.Init.CLKPhase = CONFIG_SPI1_CLKPHASE;
    V_config_spi1.Init.CLKPolarity = CONFIG_SPI1_CLKPOLARITY;
    V_config_spi1.Init.CRCCalculation = CONFIG_SPI1_CRCCALCULATION;
    V_config_spi1.Init.CRCPolynomial = CONFIG_SPI1_CRCPOLYNOMIAL;
    V_config_spi1.Init.DataSize = CONFIG_SPI1_DATASIZE;
    V_config_spi1.Init.Direction = CONFIG_SPI1_DIRECTION;
    V_config_spi1.Init.FirstBit = CONFIG_SPI1_FIRSTBIT;
    V_config_spi1.Init.NSS = CONFIG_SPI1_NSS;
    V_config_spi1.Init.TIMode = CONFIG_SPI1_TIMODE;

    V_config_spi1.Init.Mode = CONFIG_SPI1_MODE;
    FUNC_CHECK_FOR_HAL(HAL_SPI_Init(&V_config_spi1), "ERROR SPI1_Init");

    __HAL_SPI_ENABLE(&V_config_spi1);
}

void FuncSPI1NSSReady(GPIO_TypeDef *m_gpio_group, uint16_t m_gpio_pin)
{

    if (V_config_spi1.Init.CLKPolarity == SPI_POLARITY_LOW)
    {
...
{
    FuncSPI1WaitForReady();
    HAL_SPI_Transmit(&V_config_spi1, m_buffer_ptr, m_num_to_write, CONFIG_SPI1_TIMEOUT);
    FuncSPI1WaitForReady();
}
...
{
    FuncSPI1WaitForReady();
    HAL_SPI_Receive(&V_config_spi1, m_buffer_ptr, m_num_to_read, CONFIG_SPI1_TIMEOUT);
    FuncSPI1WaitForReady();
}
...
{
    /*等待传输结束*/
    FUNC_WAITSTATUS_TIMEOUT(HAL_SPI_GetState(&V_config_spi1), HAL_SPI_STATE_READY, CONFIG_SPI1_TIMEOUT, "Timeout SPI1");
}